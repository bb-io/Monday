...
public class DataSourceItemsTestHandler : BaseInvocable, IDataSourceItemHandler
{
    public DynamicForContextRequest Input { get; set; }

    public DataSourceItemsTestHandler(InvocationContext invocationContext,
    ...
    public IEnumerable<DataSourceItem> GetData(DataSourceContext context)
    {
        Dictionary<string, string> dictionary = new()
        {
            ...
            {
                "Multiple Texts",
                string.IsNullOrEmpty(string.Join(',', Input.PrimitiveArrayStrings ?? new List<string>()))
                    ? "empty"
            ...
            {
                "Boolean Optional",
                Input.BooleanOptional != null ? Input.BooleanOptional.ToString() : "empty"
            },
            { "Date Optional", Input.DateOptional != null ? Input.DateOptional.ToString() : "empty" },
            {
                ...
                Input.PrimitiveArrayOptional != null
                    ? string.Join(',', Input.PrimitiveArrayOptional ?? new List<string>())
                    : "empty"
            }
        };
